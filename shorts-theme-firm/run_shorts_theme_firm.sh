...
# ==============================================================================
# Shorts-Theme-Firm Independent Pipeline
#
# Flow:
...
UPLOAD=1
PRIVACY="public"
SHORTS_DURATION_SECONDS="${SHORTS_THEME_FIRM_DURATION_SECONDS:-90}"
SHORTS_TTS_VOICE="${SHORTS_THEME_FIRM_TTS_VOICE:-Charon}"
SHORTS_TTS_TEMPERATURE="${SHORTS_THEME_FIRM_TTS_TEMPERATURE:-0.6}"
REMOTION_TIMEOUT_MS="${SHORTS_THEME_FIRM_REMOTION_TIMEOUT_MS:-180000}"
REMOTION_CONCURRENCY="${SHORTS_THEME_FIRM_REMOTION_CONCURRENCY:-1}"
PREVIEW_SECONDS=""
PROMPT_CONFIG_PATH="${ROOT_DIR}/shorts-theme-firm/prompt/shorts_theme_firm_pipeline.yaml"
SLIDES_PROMPT_CONFIG_PATH="${SHORTS_THEME_FIRM_SLIDES_PROMPT_CONFIG:-${ROOT_DIR}/shorts-theme-firm/prompt/shorts_theme_firm_slides.yaml}"
CURRENT_STEP="init"
USE_UV=0
PYTHON_BIN="python3"
...
usage() {
    cat <<'EOF'
Usage:
  ./shorts-theme-firm/run_shorts_theme_firm.sh YYYYMMDD --lang ko|en [--start-from 1..5] [--overwrite] [--no-upload]
...
generate_script() {
    ...
    if [ "${OVERWRITE}" -eq 1 ] || [ ! -f "${SHORTS_SCRIPT_PATH}" ]; then
        ...
        run_python "${ROOT_DIR}/shorts-theme-firm/generate_script.py" \
            "${EPISODE_BASE}" \
            --duration "${SHORTS_DURATION_SECONDS}" \
            --prompt-config "${PROMPT_CONFIG_PATH}"
    else
        echo "♻️  Reusing shorts-theme-firm script: ${SHORTS_SCRIPT_PATH}"
    fi
...
generate_audio() {
    ...
    if [ "${OVERWRITE}" -eq 1 ] || [ ! -f "${SHORTS_MP3}" ] || [ ! -f "${SHORTS_SECTION_TIMING_PATH}" ]; then
        ...
        run_python "${ROOT_DIR}/shorts-theme-firm/generate_audio.py" \
            "${DATE}" \
            --lang "${LANG}" \
        ...
        echo "♻️  Reusing shorts-theme-firm audio/timing: ${SHORTS_MP3}"
    fi
...
SHORTS_BASE="${ROOT_DIR}/podcast/${DATE}/${LANG}/shorts-theme-firm"
...
SHORTS_WEB_DATA_PATH="${ROOT_DIR}/web/public/data/shorts-theme-firm/${DATE}.json"
SHORTS_WEB_AUDIO_PATH="${ROOT_DIR}/web/public/audio/shorts-theme-firm/${DATE}.mp3"
...
echo "========================================================"
echo "🎬 Shorts-Theme-Firm Independent Pipeline Start"
echo "📅 Date: ${DATE}"
echo "🌐 Language: ${LANG}"
