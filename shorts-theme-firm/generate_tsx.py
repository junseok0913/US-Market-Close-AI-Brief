"""Generate TSX payload module for shorts-theme-firm render artifacts.

Default input:
  podcast/{date}/{lang}/shorts-theme-firm/slides.render.json

Default output:
  web/src/generated/shorts-theme-firm/{date}_{lang}.generated.tsx
"""
...
def build_default_input_path(date: str, lang: str) -> Path:
    return ROOT_DIR / "podcast" / date / lang / "shorts-theme-firm" / "slides.render.json"


def build_fallback_script_path(date: str, lang: str) -> Path:
    return ROOT_DIR / "podcast" / date / lang / "shorts-theme-firm" / "script.json"


def build_default_output_path(date: str, lang: str) -> Path:
    return ROOT_DIR / "web" / "src" / "generated" / "shorts-theme-firm" / f"{date}_{lang}.generated.tsx"


def to_export_name(date: str, lang: str) -> str:
    return f"shortsThemeFirmEpisode{date}{lang.upper()}"
...
def normalize_from_script_payload(payload: dict[str, Any], *, date: str, lang: str) -> dict[str, Any]:
    ...
    if not normalized_sections:
        ...
        normalized_sections = [
            ...
            {"name": "company_2", "text": script_text},
            {"name": "company_3", "text": script_text},
            {"name": "company_4", "text": script_text},
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Generate shorts-theme-firm TSX payload module")
    parser.add_argument("date", help="Date in YYYYMMDD or YYYY-MM-DD")
    parser.add_argument("--lang", default="ko", choices=["ko", "en"])
    parser.add_argument(
        "--input",
        type=Path,
        help="Input JSON path (default: shorts-theme-firm/slides.render.json; fallback: shorts-theme-firm/script.json)",
    )
    parser.add_argument(
        "--output",
        type=Path,
        help="Output TSX path (default: web/src/generated/shorts-theme-firm/{date}_{lang}.generated.tsx)",
    )
    parser.add_argument(
        "--export-name",
        type=str,
        help="Named export identifier (default: shortsThemeFirmEpisode{date}{LANG})",
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not input_path.exists():
        ...
        if fallback_input.exists():
            ...
            raise FileNotFoundError(f"shorts-theme-firm input not found: {input_path} (fallback: {fallback_input})")
    ...
    print(f"Generated shorts-theme-firm TSX payload: {output_path}")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"Failed to generate shorts-theme-firm TSX payload: {exc}", file=sys.stderr)
        sys.exit(1)
