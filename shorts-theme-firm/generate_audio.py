"""Generate company-section TTS for shorts-theme-firm script.

Audio strategy:
- TTS call count equals the number of expert company sections.
- Hook and closing CTA are merged into first/last expert segments.
"""
...
PIPELINE_CONFIG_PATH = ROOT_DIR / "shorts-theme-firm" / "prompt" / "shorts_theme_firm_pipeline.yaml"
...
def fallback_sections(script_text: str) -> list[dict[str, str]]:
    ...
    ranges = partition_ranges(len(middle_sentences), 4)
    while len(ranges) < 4:
    ...
    for idx in range(4):
        ...
        sections.append(
            {
                "name": f"company_{idx + 1}",
                "text": compact_text(" ".join(middle_sentences[start:end])),
            }
        )
...
def load_script(script_path: Path) -> dict[str, Any]:
    if not script_path.exists():
        raise FileNotFoundError(f"shorts-theme-firm script not found: {script_path}")
...
def extract_company_segments(script_payload: dict[str, Any], lang: str) -> list[dict[str, Any]]:
    ...
    moves = [item for item in raw_moves if isinstance(item, dict)] if isinstance(raw_moves, list) else []
    base_move = moves[0] if moves else {}
    base_ticker = normalize_ticker(base_move.get("ticker"))
    base_label = compact_text(base_move.get("name")) or base_ticker or "Company"
    ...
    company_sections = [
        ...
        if is_company_section_name(section.get("name")) and compact_text(section.get("text"))
    ]
    for idx, section in enumerate(company_sections):
        section_name = normalize_section_name(section.get("name"))
        explicit_text = compact_text(section.get("text"))
        segment_text = explicit_text or build_company_segment_text(
            lang=lang,
            label=base_label,
            day_change_display=get_metric_display(base_move.get("day_change_display"), base_move.get("day_change_pct")),
            month_change_display=get_metric_display(base_move.get("month_change_display"), base_move.get("month_change_pct")),
            reason=compact_text(base_move.get("reason")),
        )
        if not segment_text:
            continue
        output.append(
            {
                "id": len(output),
                "name": section_name or f"company_{idx + 1}",
                "ticker": base_ticker,
                "label": base_label,
                "kind": "company",
                "text": ensure_sentence_end(segment_text),
            }
        )
    ...
    if base_move:
        output.append(
            {
                "id": 0,
                "name": compact_text(base_move.get("ticker")).lower() or "company_1",
                "ticker": base_ticker,
                "label": base_label,
                "kind": "company",
                "text": ensure_sentence_end(
                    compact_text(base_move.get("spoken_text") or base_move.get("tts_text"))
                    or build_company_segment_text(
                        lang=lang,
                        label=base_label,
                        day_change_display=get_metric_display(base_move.get("day_change_display"), base_move.get("day_change_pct")),
                        month_change_display=get_metric_display(base_move.get("month_change_display"), base_move.get("month_change_pct")),
        ...
        timeline.append(
            {
                ...
                "name": name,
                "ticker": ticker,
                "kind": compact_text(segment.get("kind")) or "section",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Generate company-section TTS audio for shorts-theme-firm")
    ...
    parser.add_argument(
        ...
        type=float,
        default=float(os.getenv("SHORTS_THEME_FIRM_COMPANY_PAUSE_SECONDS", os.getenv("SHORTS_THEME_FIRM_SECTION_PAUSE_SECONDS", "0"))),
...
def main(argv: list[str] | None = None) -> int:
    ...
    logger.info("shorts-theme-firm audio generation")
    ...
    try:
        ...
        logger.info("shorts-theme-firm audio generation completed")
        return 0
    except Exception as exc:
        logger.error("Failed to generate shorts-theme-firm audio: %s", exc, exc_info=args.debug)
        return 1
