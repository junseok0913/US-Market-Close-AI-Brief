...
function main() {
  ...
  const renderTimeoutMs = toPositiveNumber(
    args.timeout ?? process.env.SHORTS_FIRM_REMOTION_TIMEOUT_MS ?? process.env.REMOTION_TIMEOUT_MS,
    180000,
  );
  const concurrency = Math.max(
    ...
    Math.floor(
      toPositiveNumber(
        args.concurrency ??
          process.env.SHORTS_FIRM_REMOTION_CONCURRENCY ??
  ...
  if (sectionTimingJsonPath && fs.existsSync(sectionTimingJsonPath)) {
    try {
      ...
      console.log(`[shorts-firm] section-timing-json: ${sectionTimingJsonPath}`);
    } catch (error) {
      console.warn(
        `[shorts-firm] warning: failed to parse section timing JSON (${sectionTimingJsonPath}): ${
  ...
  console.log(`[shorts-firm] episode-json: ${episodeJsonPath}`);
  console.log(`[shorts-firm] output: ${outputPath}`);
  console.log(`[shorts-firm] fps: ${fps}`);
  console.log(`[shorts-firm] duration: ${targetDuration.toFixed(3)}s (${frameCount} frames)`);
  console.log(`[shorts-firm] frames: ${frameRange}`);
  console.log(`[shorts-firm] timeout: ${Math.round(renderTimeoutMs)}ms`);
  console.log(`[shorts-firm] concurrency: ${concurrency}`);
  if (browserExecutable) {
    console.log(`[shorts-firm] browser-executable: ${browserExecutable}`);
  }
  console.log(
    `[shorts-firm] remotion command: ${remotion.cmd} ${[...remotion.argsPrefix, "render"].join(" ")}`,
  );
  ...
  console.log(`[shorts-firm] saved: ${outputPath}`);
}

try {
  main();
} catch (error) {
  console.error(`[shorts-firm] Failed: ${error instanceof Error ? error.message : String(error)}`);
  process.exit(1);
}
