"""Generate pre-render TSX payload for YouTube shorts workflow.

Default input:
  podcast/{date}/{lang}/shorts/slides.render.json

Default output:
  web/src/generated/shorts/{date}_{lang}.generated.tsx
"""
...
def build_default_input_path(date: str, lang: str) -> Path:
    return ROOT_DIR / "podcast" / date / lang / "shorts" / "slides.render.json"


def build_default_script_input_path(date: str, lang: str) -> Path:
    return ROOT_DIR / "podcast" / date / lang / "shorts" / "script.json"


def build_default_output_path(date: str, lang: str) -> Path:
    return ROOT_DIR / "web" / "src" / "generated" / "shorts" / f"{date}_{lang}.generated.tsx"


def to_export_name(date: str, lang: str) -> str:
    return f"shortsEpisode{date}{lang.upper()}"
...
def normalize_episode_from_shorts_script(payload: dict[str, Any], date: str, lang: str) -> dict[str, Any]:
    ...
    if not normalized_sections:
        normalized_sections = [
            {"name": "hook", "text": compact_text(payload.get("hook")) or compact_text(payload.get("script"))},
            {"name": "data", "text": compact_text(payload.get("script"))},
            {"name": "story", "text": compact_text(payload.get("script"))},
            {"name": "closing", "text": compact_text(payload.get("script"))},
        ]
    ...
    for idx, section in enumerate(normalized_sections):
        ...
        phase = SECTION_TO_PHASE.get(section["name"], "insight")
    ...
    return {
        ...
        "sourceDigest": [],
        "meta": {
            "keyPoints": [compact_text(item) for item in (meta.get("key_points") or []) if compact_text(item)][:4],
            "featuredTickers": featured_tickers,
            "sceneCount": len(slides),
            "companyMoves": meta.get("company_moves") if isinstance(meta.get("company_moves"), list) else [],
        },
    }
...
def load_episode_payload(input_path: Path, date: str, lang: str) -> dict[str, Any]:
    ...
    if isinstance(raw, dict):
        return normalize_episode_from_shorts_script(raw, date=date, lang=lang)
    raise ValueError(f"Invalid input JSON payload: {input_path}")
...
def save_tsx_payload(*, episode: dict[str, Any], output_path: Path, export_name: str, source_path: Path) -> None:
    ...
    content = (
        "/* Auto-generated by shorts/generate_shorts_tsx.py. */\n"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Generate TSX payload module for shorts rendering")
    parser.add_argument("date", help="Date in YYYYMMDD or YYYY-MM-DD")
    parser.add_argument("--lang", default="ko", choices=["ko", "en"])
    parser.add_argument(
        "--input",
        type=Path,
        help="Input JSON path (default: podcast/{date}/{lang}/shorts/slides.render.json; fallback script.json)",
    )
    parser.add_argument(
        "--output",
        type=Path,
        help="Output TSX path (default: web/src/generated/shorts/{date}_{lang}.generated.tsx)",
    )
    parser.add_argument(
        "--export-name",
        type=str,
        help="Named export identifier (default: shortsEpisode{date}{LANG})",
    )
    ...
    if not input_path.exists():
        fallback_input = build_default_script_input_path(date, lang)
        if fallback_input.exists():
            input_path = fallback_input
        else:
            raise FileNotFoundError(f"Shorts input not found: {input_path} (fallback: {fallback_input})")
    ...
    print(f"Generated TSX payload: {output_path}")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"Failed to generate TSX payload: {exc}", file=sys.stderr)
        sys.exit(1)
